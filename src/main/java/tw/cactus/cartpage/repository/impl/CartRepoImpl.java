package tw.cactus.cartpage.repository.impl;
...
public class CartRepoImpl implements CartRepo{
	...
	CactusBean cautusBean;
	
	// 購物車頁面 從DATABASE撈出資料
	public List<CartBean> findByUserId(int userid) {
	String hql = "from CartBean where USER_ID=:'USERID'";
	List<CartBean> cab = (List<CartBean>) entityManager.createQuery(hql, CartBean.class)
										.setParameter("USERID", userid).getResultList();
		return cab;
	}

	@Override
	public List<CartBean> findCartList(Integer userId) {
		String hql ="from cart where USER_ID = :userId";
		List<CartBean> cartList = entityManager.createQuery(hql, CartBean.class).setParameter("userId", userId).getResultList();
		return cartList;
	}
	...
	public void adddel(Integer userid,List<CartBean> c) {
			...
			cautusBean.setUserId(userid);
			for (int j = 0; j < c.size()-1; j++) {
					sessionBean.setCactusBean(cautusBean);
					sessionBean.setCourseBean(c.get(j).getCourseBean());
